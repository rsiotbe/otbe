    ...
    public static ArrayList<MiqValidation> getDDBBValidations(String strPathRest) throws Exception
    {
        ...
        try
        {
            String strQuery = "select * from " + " " + AppConfiguration.getInstance().getProperty("BELScheme").trim()
                    + ".bdptb222_miq_quests z," + " " + AppConfiguration.getInstance().getProperty("BELScheme").trim()
                    + ".bdptb226_miq_quest_rl_session x," + " "
                    + AppConfiguration.getInstance().getProperty("BELScheme").trim()
                    + ".bdptb225_miq_session_params a," + " "
                    + AppConfiguration.getInstance().getProperty("BELScheme").trim()
                    + ".BDPTB228_MIQ_PARAM_VALIDATION b" + " where z.path_rest=? and z.id_miq=x.id_miq"
                    + " and x.id_miq_param= a.id_miq_param" + " and a.id_miq_param=b.id_miq_param";
