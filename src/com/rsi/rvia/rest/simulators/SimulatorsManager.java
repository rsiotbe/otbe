...
public class SimulatorsManager
{
            ...
            String strLanguage) throws Exception
    {
        ...
        try
        {
            ...
            // TODO: Debería filtrarse por categoría.
            // String[] categories = Arrays.toString(Constantes.SimuladorCategorias.values()).replaceAll("\\[|\\]",
            // "").split(", ");
            // OracleConnection oraConn = pConnection.unwrap(OracleConnection.class);
            // Array inCategories = oraConn.createArrayOf("VARCHAR2", cat);
            // pPreparedStatement.setArray(6, inCategories);
            if (strSimulatorName != null && !strSimulatorName.trim().isEmpty()
                    && !"null".equals(strSimulatorName.trim()))
            {
