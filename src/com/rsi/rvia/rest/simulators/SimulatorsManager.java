...
public class SimulatorsManager
{
			...
			String strSimulatorName, String strLanguage) throws Exception
	{
		...
		String[] astrCategories = null;
		Array pOracleInArray;
		try
		{
			...
			{
				case MORTGAGE:
					/* se obtienen todos los tipos posibles de prestamo de hipoteca */
					(Constantes.SimulatorMortgageCategory.values()).replaceAll("\\[|\\]", "");
					break;
				case PERSONAL:
					/* se obtienen todos los tipos posibles de prestamo personal */
					astrCategories = Arrays.toString(Constantes.SimulatorLoanCategory.values()).replaceAll("\\[|\\]", "").split(", ");
					break;
				default:
					break;
			}
			...
			/* se genera el array para el IN de categorias con los datos de las enumeraciones de tipos */
			OracleConnection pOracleConnection = pConnection.unwrap(OracleConnection.class);
			pOracleInArray = pOracleConnection.createArrayOf("VARCHAR2", astrCategories);
			pPreparedStatement.setArray(6, pOracleInArray);
			if (strSimulatorName != null && !strSimulatorName.trim().isEmpty() && !"null".equals(strSimulatorName.trim()))
			{
